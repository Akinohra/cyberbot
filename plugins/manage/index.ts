...
import { type Plugin, Structs, ctx, NodeSegment, napcat } from "../../core/index.js";
...
const plugin: Plugin = {
  ...
  handlers: {
    message: async (e) => {
      ...
      // 判断是否有权限
      if((await ctx.hasRight(e.sender.user_id))){
        // at 消息元素列表
        const ats = ctx.getMessageAt(e);
        //根据空格拆分消息
        const messages:string[] = commond.split(/\s+/);
        if('group_id' in e && commond.match(regex)) {
          ...
          if (commond.startsWith('#mg')) {
            ...
            if (condition) {
              ...
              return await ctx.reply(e, successMsg);
            }
          
            return ctx.reply(e, errorMsg);
          }
          // 群聊触发
          if(!config.enableGroups.includes(e.group_id)){
            return
          ...
          if (commond.startsWith('#踢')) {
            if(!ats[0]){
            return ctx.reply(e, '❎移出失败，该群员不存在');
            }
            await ctx.kick(e.group_id, ats[0])
            return ctx.reply(e, `🌟${ats[0]} 被移出群聊`);
          }
          else if(commond.startsWith('#禁言')){
              // 执行禁言
              if(!ats[0]){
                return ctx.reply(e, '❎禁言/解除禁言失败，该群员不存在');
              }
              const info = await napcat.get_group_member_info({group_id:e.group_id, user_id: ats[0]})
              const name = info.card || (info.nickname ?? ats[0])
              ctx.ban(e.group_id, ats[0], parseInt(messages[2]) * 60);
              return ctx.reply(e, name + '已被禁言'+messages[2] + '分钟！');

          }
          else if(commond.startsWith('#解禁')){
              // 解除禁言
              if(!ats[0]){
                return ctx.reply(e, '❎解除失败，该群员不存在');
              }
              const info = await napcat.get_group_member_info({group_id:e.group_id, user_id: ats[0]})
              const name = info.card || (info.nickname ?? ats[0])
              ctx.ban(e.group_id, ats[0], 0);
              return ctx.reply(e, '✅已解除对'+name+'的禁言！');
          }
          else if (['#关灯', '#全员禁言'].includes(commond)) {
              ctx.banAll(e.group_id, true);
              return ctx.reply(e, '✅已开启全员禁言');
          }
          else if (['#开灯', '#全员解禁'].includes(commond)) {
              ctx.banAll(e.group_id, false);
              return ctx.reply(e, '✅已解除全员禁言'); 
          }
          else if(commond.startsWith('#撤回')){
              if(!ats[0]){
                return ctx.reply(e, '❎撤回失败，该消息指向的用户不存在');
              }
              let count = 0,  m_id = 0;
              ...
              return ctx.reply(e, "✅撤回成功");
          }
          else if(commond.startsWith('#改群名')){
              napcat.set_group_name({ group_id: e.group_id, group_name: messages[1] })
              return ctx.reply(e, "✅更改成功", true);
          }
          else if(commond.startsWith('#加管理')){
              if(!ats[0]){
                return ctx.reply(e, '❎添加失败，该群员不存在');
              }
              napcat.set_group_admin({ group_id: e.group_id, user_id: ats[0], enable: true });
              return ctx.reply(e, "✅添加成功", true);
          }
          else if(commond.startsWith('#删管理')){
              if(!ats[0]){
                return ctx.reply(e, "❎删除失败，该群员不存在", true);
              }
              // napcat.setGroupAdmin(e.group_id, qqs[1], false);
              napcat.set_group_admin({ group_id: e.group_id, user_id: ats[0], enable: false });
              return ctx.reply(e, "✅删除成功", true);
          }
          if(commond.startsWith('#改名片')){
              if(!ats[0]){
                return ctx.reply(e, '❎修改失败，该群员不存在');
              }
              napcat.set_group_card({ group_id: e.group_id, user_id: ats[0], card: messages[2] });
              return ctx.reply(e, "✅修改成功", true);
          }
          else if(commond.startsWith('#改头衔')){
              if(!ats[0]){
                return ctx.reply(e, '❎修改失败，该群员不存在');
              }
              napcat.set_group_special_title({ group_id: e.group_id, user_id: ats[0], special_title: messages[2] });
              return ctx.reply(e, "✅修改成功", true);
          }
        }
        if(commond.startsWith('#加撤回词')){
          if(!messages[1]){
            return ctx.reply(e, '格式错误，正确格式：#加撤回词 <词>', true);
          }
          if (config.recallwords.includes(messages[1])) {
            return ctx.reply(e, '❎ 词已存在');
          }
          config.recallwords.push(messages[1]);
          saveConfig(configPath, config)
          return ctx.reply(e, '✅ 添加成功');
        }
        else if(commond.startsWith('#删撤回词')){
          if(!messages[1]){
            return ctx.reply(e, '格式错误，正确格式：#删撤回词 <词>', true);
          }
          if (!config.recallwords.includes(messages[1])) {
            return ctx.reply(e, '❎ 词不存在');
          }
          const idx = config.recallwords.findIndex(e => e[0] === messages[1]);
          config.recallwords.splice(idx, 1);
          saveConfig(configPath, config)
          return ctx.reply(e, '✅ 删除成功');
        }
        else if(commond.startsWith('#加禁言词')){
          if(!messages[1]){
            return ctx.reply(e, '格式错误，正确格式：#加禁言词 <词>', true);
          }
          ...
          if(!messages[1]){
            return ctx.reply(e, '格式错误，正确格式：#删禁言词 <词>', true);
          }
          if (!config.banwords.includes(messages[1])) {
            ctx.reply(e, `${messages[1]}`);
            return ctx.reply(e, '❎ 词不存在');
          }
          ...
          if(config.banwords.length === 0){
            return ctx.reply(e, '禁言词列表为空', true);
          }
          const target_id: number = 'group_id' in e ? e.group_id : (e as PrivateFriendMessage).user_id;
          // 禁言词列表展示逻辑
          ...
          if(config.recallwords.length === 0){
            return ctx.reply(e, '撤回词列表为空', true);
          }
          const target_id: number = 'group_id' in e ? e.group_id : (e as PrivateFriendMessage).user_id;
          // 禁言词列表展示逻辑
          const forwardmsg: NodeSegment[] = [
        ...
        for (const item of config.banwords) {//精确
          ...
          if ( raw_message !== item && isRegexString(item)) {
            ...
            if (content) {
              ...
              const { message_id:mid } = await ctx.reply(e, '消息含有违禁词，请文明聊天。');
              // 60s撤回
              return setTimeout(() => {
                  ctx.delete_msg(mid);
              }, 10 * 1000);
